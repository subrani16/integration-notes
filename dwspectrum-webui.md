---
title: DW Spectrum Integration Note v1.1.0
logo: assets/images/dw-logo.png
---
...
-   DW Spectrum VMS version 6.0 or greater.
...
In this web UI integration, the DW Spectrum VMS receives the annotated RTSP stream from the VCAserver and the alarms are
sent through the HTTP action with VCA tokens containing details about the event.

![](assets/images/dw-architecture.png)
...
4.  Note the **Channel ID** as this will be needed when connecting to the RTSP stream from the DW server.
...
5.  Click the arrow on the right of the action to expand the HTTP configuration options.
    ...
    -   **URI:** Enter the `CreateEvent` API call required to trigger an event of the 'Generic Event' type in the
        DW system. Example: `https://<IP_address:7001>/api/createEvent`.
    -   **Port:** Enter the port number used for sending the notification to the DW server.
    ...
    select the logical rule created for the source you want to send to the VMS.
        ...
        ![](assets/images/dw-vca-http-action.png)
...
![](assets/images/dw-add-device.png)
...
1.  In the **Add Devices** window, configure the VCA RTSP stream as follows:
    ...
    -   Select the new device and Click **Add all devices**.
        ...
        ![](assets/images/dw-vca-rtsp-config.png)

    _Note: Additionally, you can rename the device by right click and selecting Rename._

### Configuring the Recording

1.  Right click on the camera screen and select **Camera Settings**.

    ![](assets/images/dw-camera-settings.png)

2.  In the pop-up screen, click **Recording** and **enable** the option.

    ![](assets/images/dw-recording-config.png)
...
3.  **Copy** the string and **close** the window.
    ...
    ![](assets/images/dw-vca-camera-id.png)

## Generating the `CreateEvent` API call

The `CreateEvent` API call allows 3rd party systems to send Generic Events to the DW Spectrum Server for use in the
...
-   **Address**: Indicates the IP address of the DW server.
-   **Port**: Indicates port of the DW server.
...
```json
{
    ...
    "metadata": {
        "cameraRefs": [
            "ea39ce1e-6367-4c45-d2c6-637ffa2bad58"
        ]
    }
}
```
...
-   **`cameraRefs`:** Camera ID registered in DW Spectrum server. Specifies the list of Devices which are linked to the
    Event.
...
For more details about authentication of third-party integrations and how to get a Session Token please refer to DW
Spectrum API Documentation -
[Authentication](https://dwspectrum.digital-watchdog.com/docs/developers/knowledgebase/278-how-does-authentication-work)

### Configuring the Camera Rules in the Event Rules

Next, we configure the rule that will trigger a specific action when a condition is met. Right click on the camera
screen and select **Camera Rules** from the available options.

![](assets/images/dw-camera-rules.png)

1.  Click the **+Add** button located top right.

    ![](assets/images/dw-add-rule.png)

2.  Configure **Event** as follows:

    -   Click on the drop down menu for **When** and select **Generic Event**.

        ![](assets/images/dw-generic-event-menu.png)
    ...
    -   **Description Contains:** Leave empty (information sent by the VCAserver).
        ...
        ![](assets/images/dw-generic-event-config.png)

3.  Configure **Action** as follows:

    -   Click on the drop down menu for **Do** and select **Show desktop notification**.

        ![](assets/images/dw-notifications-menu.png)

    -   Deselect **Interval of action**.
    -   Click **OK** to save the configuration.

        ![](assets/images/dw-notifications-config.png)

### Verifying VCA Events

The generic events will appear in the NOTIFICATIONS and EVENTS tabs located on the right hand side as follows:

-   **NOTIFICATIONS**

    ![](assets/images/dw-vca-notifications.png)

-   **EVENTS**

    ![](assets/images/dw-vca-events.png)
