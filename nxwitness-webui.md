---
title: Nx Witness Integration Note v1.1.2
...
-   HTTP & JSON metadata available via tokens.
-   Annotated RTSP stream.

## Architecture

In this web UI integration, the Nx Witness VMS receives the annotated RTSP stream from the VCAserver and the alarms are
sent through the HTTP action with VCA tokens containing details about the event.

![](assets/images/nx-architecture.png)
...
4.  Note the **Channel ID** as this will be needed when connecting to the RTSP stream from the Nx Witness server.
...
5.  Click the arrow on the right of the action to expand the HTTP configuration options.
    ...
    -   **Port:** Enter the port number used for sending the notification to the NX Witness server.
    ...
    ![](assets/images/nx-vca-http-action.png)
...
-   `{{#Object}}{{#DLClassification}}{{class}}{{/DLClassification}}{{/Object}}`  
     The Deep Learning classification of the object.
-   `{{type.string}}`  
     The type of the event (usually the type of rule that triggered the event).
...
select **Add** from the menu and click **Devices** from the available options.

![](assets/images/nx-add-device.png)

In the **Add Devices** window, configure the VCA RTSP stream as follows:

-   **Address:** Enter the RTSP URL of the VCA channel. The default format is:

    `rtsp://<device ip>:<RTSP_port>/channels/<channel id>`.

-   **Port:** Unchecked the default option and enter the RTSP port configured in the VCA device.
...
-   Select the new device and Click **Add all devices**.
    ...
    ![](assets/images/nx-vca-rtsp-configuration.png)
...
3.  **Copy** the string and **close** the window.
    ...
    ![](assets/images/nx-vca-camera-id-string.png)

### Configuring the Recording

1.  Right click on the camera screen and select **Camera Settings**. In the pop-up screen, click **Recording** and
    **enable** the option.

    ![](assets/images/nx-recording.png)

    Optionally, you can schedule the recording of events if required. To do this, select the type of recording (Always
    or Motion Only) for the days.
...
```json
{
    ...
    "metadata": {
        "cameraRefs": [
            "13bbe255-ab22-2031-2144-01aa6d8874b4"
        ]
    }
}
```
...
Nx Witness uses a bearer token for authentication. Here is a breakdown of the common steps required to generate an API
Token depending on the authentication method, for detailed information, please refer to the Nx Witness documentation.
...
For more details about authentication of third-party integrations and how to get a Session Token please refer to Nx
Witness API Documentation.

## Configuring the Camera Rules in the Event Rules

Next, configure the rule that will trigger a specific action when a condition is met. Right clicking on the camera
...
2.  Configure **Event** as follows:
    ...
    -   **Description Contains:** Leave empty (information sent by the VCAserver).
        ...
        ![](assets/images/nx-generic-events-config.png)
